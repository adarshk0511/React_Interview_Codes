...
const MemoryGame = () => {
  ...
  const [flipped, setFlipped] = useState<(string | number)[]>([]);
  ...
  const checkMatch = (secondId: any) => {
    const [firstId] = flipped;
    if (cards[Number(firstId)].number === cards[secondId].number) {
  ...
  const isFlipped = (id: any) => flipped.includes(id) || solved.includes(id);
  const isSolved = (id: any) => solved.includes(id);
